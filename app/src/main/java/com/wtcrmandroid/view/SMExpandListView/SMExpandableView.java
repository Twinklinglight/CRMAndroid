 ...
 *         ExpandableListView侧滑的自定义布局
 */
public class SMExpandableView extends ExpandableListView {
	...
	private SMLayoutView mTouchView;
	...
	public SMExpandableView(Context context) {
		super(context);
		init();
	}

	public SMExpandableView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init();
	}

	public SMExpandableView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	public void setAdapter(ExpandableListAdapter adapter) {
		super.setAdapter(new SMExpandableAdapter(getContext(), adapter) {
			@Override
			...
			@Override
			public void onItemClick(SMExpandView view, SwipeMenu menu,
									int index) {
				boolean flag = false;
		...
		switch (action) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (!mTouchView.getSwipEnable() || mTouchPosition != mTouchView.getPosition()) {
					if (!mState && (mTouchPosition != (mExpandCount + 1))) {
	...
	public void setMenuCreator(SwipeMenuCreatorInterfaceUtil menuCreator) {
